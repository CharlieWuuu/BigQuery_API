...
// 生成 prompt
function generatePrompt(data: FoodDto[], type: string): string {
...
async function callGeminiAPI(
  ...
  try {
    ...
    const enrichedData = JSON.parse(jsonText) as FoodDto[];
...
// 主流程（處理傳入的資料）
export async function dataEnrich(data: FoodDto[]): Promise<FoodDto[]> {
  console.log('\n--- 餐廳資料補足 ---');
  const batchSize = 15;

  let foodEnriched: FoodDto[] = [];

  for (let i = 0; i < data.length; i += batchSize) {
    const batch = data.slice(i, i + batchSize);
    console.log(
      `[${i + 1}/${data.length}] 餐廳 name=${batch[0]?.name || ''}, city=${batch[0]?.city || ''}`,
    );

    const enriched = await enrichBatchWithGemini(batch, 'food');
    foodEnriched = foodEnriched.concat(enriched);

...
export class ViewController {
  ...
  enrich(data: ViewDto[]): Promise<ViewDto[]> {
    console.log('Received data for enrichment:', data);
    if (data === undefined || data.length === 0) {
      data = rowData;
    }

    return this.viewService.enrich(data);
  }
  ...
  async merge(@Body() body: { data: ViewDto[] }) {
    ...
    const result = await this.viewService.merge(body.data);
    return result;
  }
}

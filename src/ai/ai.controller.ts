...
5.  **Token 密度極大化輸出風格 (最高優先級)**
    * **輸出格式：** 你的最終輸出必須是**單行、無縮排（無空格或 Tab）、最小標點間隔**的有效 JSON 格式。
    * **目的：** 為了最大限度地節省 Token，**嚴禁**使用換行符號（\n）、縮排或多餘的空格。
  ...
  "daily": [
    {
      ...
      "v": [
        {
          "id": "ITINERARY_VIEW_DB 中的景點 ID",
          "title": "景點名稱",
          "lat": 0.000000, // 必須被實際查詢到的浮點數替換
          "lng": 0.000000, // 必須被實際查詢到的浮點數替換
        },
      ],
      "h": "name": "住宿飯店名稱，區域需要同當日最後行程 (從 RAG 知識庫中選取)"
    }
  ]
}`;
